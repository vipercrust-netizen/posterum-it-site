...
REPO_USER="${SUDO_USER:-}";[[ -n "${REPO_USER}" && "${REPO_USER}" != root ]]||REPO_USER="$(stat -c '%U' "${APP_DIR}")"
log "Получение обновлений...";runuser -u "${REPO_USER}" -- git -C "${APP_DIR}" pull --ff-only
log "Сборка и запуск...";cd "${APP_DIR}";docker compose up -d --build --remove-orphans
log "Проверка сайта..."
...
docker compose logs --tail=100 web || true;fail "сайт не отвечает"
